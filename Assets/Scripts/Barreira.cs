...
void Start() {
    ...
    DeleteWallSection(tilemap, onde,qual,randomWall, Player);
}   

 void DeleteWallSection(Tilemap tilemap, int startIndex,  int x, int randomWall, GameObject Player)
    {
    if (randomWall == 0){
        for (int i = startIndex; i <= startIndex+holeSize; i++){tilemap.SetTile(new Vector3Int(x, i, 0), null);}
        Spawn.gameObject.transform.position =  new Vector3((x+42-15)*((float) 0.3), (startIndex+24+15)*((float) 0.3), 0);
        Player.transform.position =  new Vector3((x)*((float) 0.3)-3, (startIndex)*((float) 0.3)+1  , 0);
        }
    else if (randomWall == 1){
        for (int i = startIndex; i <= startIndex+holeSize; i++){tilemap.SetTile(new Vector3Int(i, x-1, 0), null);}
        Spawn.gameObject.transform.position =  new Vector3((startIndex+42)*((float) 0.3), (x+24+15)*((float) 0.3), 0);
        Player.transform.position =  new Vector3((startIndex)*((float) 0.3)+1,(x)*((float) 0.3)+3  , 0);

        }
    else if (randomWall == 2){
        for (int i = startIndex; i <= startIndex+holeSize; i++){tilemap.SetTile(new Vector3Int(x-1, i, 0), null);}
        Spawn.gameObject.transform.position =  new Vector3((x+42)*((float) 0.3), (startIndex+24+15)*((float) 0.3), 0);
        Player.transform.position =  new Vector3((x)*((float) 0.3)+3, (startIndex)*((float) 0.3)+1  , 0);
        ...
        Spawn.gameObject.transform.position =  new Vector3((startIndex+42)*((float) 0.3), (x+24)*((float) 0.3), 0);
        Player.transform.position =  new Vector3((startIndex)*((float) 0.3)+1,(x)*((float) 0.3)-3  , 0);
